...
exports.command = (cmd, args, input, message) => {
	...
	if (message.channel.type == "dm" || message.channel.nsfw)
	{
		if (!input)
		{
			...
			Nekos.nsfw[type]().then((nsfw) => message.channel.send(
		...
		if (input.startsWith('['))
			message.channel.send("With***out*** the brackets. How is that ***not*** obvious? You're probably too young to look at porn, go play violent video games, instead.");
		else
		...
		if (["random"].includes(args[0]))
		{
			...
			else
			{
				...
				if (types.length + nopes.length > tags.length)
					message.channel.send("There's not even that many tags, try again.");
				else
				if (types.length > 0)
				{
					type = types.random();
					...
					message.channel.send("None of those tags exist.");
			}
		...
		if (["exclude", "remove"].includes(args[0]))
		{
			...
			else
			{
				...
				if (types.length == tags.length)
					message.channel.send("That removes literally every tag, try again.");
				else
				if (types.length > 0)
				{
					let Types = [];
		...
		else
		{
			...
			tags.some(tag => tag.toLowerCase().includes(args[0].toLowerCase())) ?
				...
				false;
			if (!type)
				message.channel.send("Sorry, I don't have that!");
			else
				Nekos.nsfw[type]().then(nsfw => message.channel.send(
					new Discord.MessageEmbed()
						.setAuthor("x!nsfw", "https://raw.githubusercontent.com/Xyvyrianeth/xyvybot/master/assets/misc/nekos_life.png")
