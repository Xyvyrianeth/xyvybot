...
export const command = async (interaction) => {
    ...
    if (command[1] == "start" && command[2] == "accept")
    {
        ...
        if (!Game)
        {
            return await interaction.reply({ content: "Sorry, there's not a game for you to accept anymore", ephemeral: true });
        }

        if (Game.started)
        {
            return await interaction.reply({ content: "This game has already been accepted by another player", ephemeral: true });
        }

        if (interaction.user.id == interaction.message.embeds[0].description.match(/[0-9]+/))
        {
            return await interaction.reply({ content: "You cannot play against yourself", ephemeral: true });
        }
    ...
    {
        const author = { name: "author.png", attachment: `./assets/authors/${gamename}.png` };
        const embed = {
    ...
    if (command[1] == "start" && command[2] == "global")
    {
        ...
        {
            return await interaction.reply({ content: "You are already playing a game. Finish it before you start a new one.", ephemeral: true });
        }
        if (Games.some(Game => Game.players.includes(interaction.user.id) && !Game.started))
        {
            return await interaction.reply({ content: "You are already queuing for a game.", ephemeral: true });
        }
