...
var timers = setInterval(function() {
	db.query("SELECT * FROM timers", (err, res) => {
		if (err)
			return sqlError("Internal (var timers @ line 23)", err, "SELECT * FROM timers");
		...
		if (/^(connect4|squares|othello|rokumoku|ttt3d|ordo|soccer)_[0-2]_[0-9]{1,}(|vs[0-9]{1,})\.png$/.test(img))
		{
			...
			if (result)
				db.query(`SELECT *\n` + `FROM profiles\n` + `WHERE id = '${result.winner}' OR id = '${result.loser}'`, (err, res) => {
						...
						for (let i = 0; i < 8; i++)
							if (
								wins.elo1 + wins.elo2 + wins.elo3 + wins.elo4 + wins.elo5 + wins.elo6 + wins.elo7 >= [15E3, 2E4, 25E3, 3E4, 4E4, 5E4, 75E3, 1E5][i] &&
								!wins.titles.includes(["15k_elo", "20k_elo", "25k_elo", "30k_elo", "40k_elo", "50k_elo", "75k_elo", "100kelo"][i])
							)
						...
						for (let i = 0; i < 8; i++)
							if (
								lose.elo1 + lose.elo2 + lose.elo3 + lose.elo4 + lose.elo5 + lose.elo6 + lose.elo7 <= [2500, 2E3, 1500, 1E3, 750, 500, 250, 0][i] &&
								!lose.titles.includes(["2500elo", "2000elo", "1500elo", "1000elo", "750_elo", "500_elo", "250_elo", "eloGone"][i])
							)
...
var commands = {
				...
				if (res[0].rows.length > 0)
				{
					...
					for (let i = 0; i < top.length; i++) {
						...
						let place = top[i].place,
							...
							win = top[i].win,
							los = top[i].los,
							w_l = win + los > 0 ? (win / (win + los) * 100).toFixed(2) + '%' : "\u034f \u034f N/A \u034f \u034f";
	...
	"game": (cmd, args, input, message, sendChat) => {
		...
		condition = (condition) => {
			return {
				...
				"noGameHere":		 (game) => { return game.game == gameName && !game.channels.hasOwnProperty(message.channel.id); },
				"gameHere":			 (game) => { return game.game == gameName && !game.started && game.channels.hasOwnProperty(message.channel.id); },
				"gameThere":		 (game) => { return game.game == gameName && !game.started && !game.channels.hasOwnProperty(message.channel.id) && !game.here; },
				"playingYourself":	 (game) => { return game.channels.hasOwnProperty(message.channel.id) && game.players.includes(message.author.id) && game.game == gameName && !game.started; },
				"somethingElseHere": (game) => { return game.channels.hasOwnProperty(message.channel.id) && game.players.includes(message.author.id) && game.game !== gameName && !game.started; },
				"someoneElseHere": 	 (game) => { return game.channels.hasOwnProperty(message.channel.id) && game.game !== gameName && !game.started; },
				"gameStarted":		 (game) => { return game.channels.hasOwnProperty(message.channel.id) && game.started; },
				"alreadyQueued":	 (game) => { return !game.channels.hasOwnProperty(message.channel.id) && game.players.includes(message.author.id) && game.game == gameName; },
				"nothingHere":		 (game) => { return game.channels.hasOwnProperty(message.channel.id); },
				"participant":		 (game) => { return game.channels.hasOwnProperty(message.channel.id) && game.players.includes(message.author.id); },
				"dontStart":		 (game) => { return game.channels.hasOwnProperty(message.channel.id) && game.players.includes(message.author.id) && !game.started; },
				"quit":				 (game) => { return game.channels.hasOwnProperty(message.channel.id) && game.players.includes(message.author.id) && game.started; }
			}[condition];
		}
	...
	"profile": (cmd, args, input, message, sendChat) => {
		if (!input || /^<@[0-9]{1,}>$/.test(input) || /^[0-9]{1,}$/.test(input))
		{
			...
			return db.query(`SELECT * FROM profiles WHERE id = '${member.id}'`, (err, res) => {
				if (err)
