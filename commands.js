				...
				db.query([
					...
					if (err)
					{
						sqlError(message, err,
							[	`SELECT *`,
								`FROM profiles`,
								`WHERE id = '${result.winner}' OR id = '${result.loser}'`].join('\n'));
					}
					let wins;
					let lose;
	...
	"games": function(cmd, args, input, message, sendChat) {
		if (!args[0])
		{
			let embed = new Discord.RichEmbed()
				.setDescription("Input tree for command x!games\n`x!games [input]`")
				.addField("leaderboard", "View the players with the 10 highest Elos for every game or the players with the 10 highest combined Elos.\n`x!games leaderboard [game]`\nLeave `[game]` blank for general top 10 players.")
				.addField("stats", "View either your stats or another player's stats.")
				.addField("info", "A detailed information about how the Elo system works and the entire ranking system in general.")
				.addField("games", "A list of all the games that are a part of the ranking system and a few details about them.")
				...
				case "othello":
				{
					embed.setDescription(
						[	`Othello, or Reversi, is an ${wiki.asg}. The game consists of stones of both black and white and is played on an 8x8 board. The game starts with 2 stones of each color placed in the middle of the board with stones of the same color diagonal to each other. Black goes first.`,
							...
							"The obective of the game is to have the most stones on the board before both player can no longer make a legal move."].join('\n'))
					.addField(
						"Stone Placement",
				...
				case "squares":
				{
					embed.setDescription(
						[	`Squares is an ${wiki.asg} created by Xyvyrianeth originally for Xyvybot. The game consists of stones of both black and red and is played on a 10x10 board.`,
							...
							"The objective of the game is to arrange as many sets of 4 stones into perfect squares as possible, the winner being the player with the most squares at the end."].join('\n'))
					.addField(
						"Stone Placement",
						[	"To place a stone on the board, simply say the location of the empty space; for example, say \"4C\" to place a stone in Row 4–Column C.",
				...
				case "ttt3d":
				{
					embed.setDescription(
						[	`3D Tic Tac Toe is an ${wiki.asg} and, as the name suggests, a type of ${wiki.attt}. The game consists of markers, either X's or O's, and a 3-dimensional 4x4x4 playing area.`,
							...
							"The way the 3D Tic Tac Toe playing area is displayed might be confusing for some players. Traditionally, the 4 *y*-planes are displayed vertically, but to save space, the 2nd and 4th *y*-planes are to the right of the 1st and 3rd."].join('\n'))
					.addField(
						"Marker Placement",
				...
				case "connect4":
				{
					embed.setDescription(
						[	`Connect Four is an ${wiki.asg} and a type of ${wiki.attt}. The game consists of stones of both black and red and is played on a 6x7 board. Blue goes first.`,
							...
							"This has been a very popular board game for many years, and if you've never heard of it until now, your childhood was clearly shit."].join('\n'))
					.addField(
						"Stone Placement",
				...
				case "ordo":
				{
					embed.setDescription(
						[	`Ordo is an ${wiki.asg} that has rules and gameplay reminiscent of Checkers. The game consists of stones of both blue and white and is played on an 8x10 board. The game starts with all stones on the board in a set pattern. Blue goes first.`,
							"Players take turns moving 1 stone around the board in diagonal or orthagonal directions (or multiple stones, but only orthagonally), making sure that all of their stones are connected in the same group."].join('\n'))
					.addField(
						"Singleton Moves",
		...
		if (["background", "backgrounds", "bg", "bgs"].includes(args[0]))
		{
			if (!args[1] && !["backgrounds", "bgs"].includes(args[0]))
			{
				return db.query(
					[	`SELECT *`,
						`FROM profiles`,
						`WHERE id = '${message.author.id}'`].join('\n'), function(err, res) {
					if (err)
					{
						return sendChat("```" + err + "```");
					}
			...
			if (["buy", "purchase"].includes(args[1]))
			{
				return db.query(
					...
					return db.query(
						...
						{
							return sendChat("Successfully purchased a new background! To equip it, do `x!profile background [background ID]`. New background ID: `" + newbg + '`', new Discord.Attachment("./assets/backgrounds/" + newbg.substring(0, 7) + (newbg.substring(7) == 'j' ? ".jpg" : ".png")));
						}
					});
				});
			}
	...
	"help": function(cmd, args, input, message, sendChat) {
		if (!input)
		{
			...
			let embed = new Discord.RichEmbed()
				...
				.addField("Utility", helps[1])
				.addField("Miscellaneous", helps[2])
				.setColor(new Color().random())
