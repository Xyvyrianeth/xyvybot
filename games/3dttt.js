        ...
        for (let y = 0; y < 4; y++)
        {
            for (let z = 0; z < 4; z++)
            {
                if (game.board[x + 1][(y + 10).toString(14).toUpperCase()][z] == false)
                {
					...
					// I have to do this. There's no other way. This is as simple as it gets.
	let wins = [	// Each section below would be its own algorithm if I chose to actually make algorithms for this part.
					// That's at least 9 different algorithms that return the same results. The last section likely can't even be algorithm'd.
