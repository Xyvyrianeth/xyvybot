...
const interval = () => {
	Games.forEach(async (Game, id) => {
		...
		{
			const Channel = await Xyvybot.channels.fetch(Game.channels[0]);
			const channelPerms = Channel.permissionsFor(Xyvybot.user.id);
			...
			if (!Channel || ([10, 11, 12].includes(Channel.type) && !hasPerms1) || !hasPerms2)
			{
				...
				{
					const user = await Xyvybot.users.fetch(Game.players[index]);
		...
		else
		{
			...
			{
				const Channel = await Xyvybot.channels.fetch(Game.channels[index]);
				const channelPerms = await Channel.permissionsFor(Xyvybot.user.id);
				const hasPerms1 = await channelPerms.has([1n << 10n, 1n << 38n]);
				const hasPerms2 = await channelPerms.has([1n << 10n, 1n << 11n]);
				if (!Channel || ([10, 11, 12].includes(Channel.type) && !hasPerms1) || !hasPerms2)
				{
